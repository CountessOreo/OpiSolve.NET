...
namespace OptiSolver.NET.Core
{
    ...
    public class LPModel
    {
        ...
        // Maximization or <inimization problem
        ...
        public bool IsBinaryProgram => Variables != null && Variables.Count > 0 && Variables.All(v => v.Type == VariableType.Binary);
        ...
        {
            // Ensure coefficients list is sized correctly
            var count = coefficients?.Count ?? 0;
            if (count > Variables.Count)
                throw new ArgumentException($"Too many coefficients: {count} > number of variables {Variables.Count}");
        ...
        public void ValidateModel()
        {
            ...
            if (Constraints == null || Constraints.Count == 0)
                errors.Add("Model must have at least one constraint");
        ...
        {
            if (variableValues == null || variableValues.Length != Variables.Count)
                return false;

            // Check variable bounds implied by VariableType
            for (int i = 0; i < Variables.Count; i++)
            {
                var (lo, hi) = EnumHelper.GetDefaultBounds(Variables[i].Type);
                if (variableValues[i] < lo - tolerance || variableValues[i] > hi + tolerance)
                    return false;
            }
        ...
        public bool IsCurrentSolutionFeasible(double tolerance = 1e-9)
        {
            ...
            {
                var (lo, hi) = EnumHelper.GetDefaultBounds(Variables[i].Type);
                if (Variables[i].Value < lo - tolerance || Variables[i].Value > hi + tolerance)
                    return false;
            }
        ...
        /// </summary>
        public List<(int constraintIndex, double violation)> GetViolations(double[] x)
        {
            var violations = new List<(int, double)>();
            if (x == null || x.Length != Variables.Count)
                return violations;

            // Variable bound violations
            for (int v = 0; v < Variables.Count; v++)
            {
                var (lo, hi) = EnumHelper.GetDefaultBounds(Variables[v].Type);
                if (x[v] < lo - 1e-9)
                    violations.Add((-(v + 1), lo - x[v]));           // negative index for var bound
                if (x[v] > hi + 1e-9)
                    violations.Add((-(v + 1), x[v] - hi));
            }

            // Constraint violations
            for (int c = 0; c < Constraints.Count; c++)
            {
                double viol = Constraints[c].GetViolation(x);
                if (viol > 1e-9)
                    violations.Add((c, viol));
            }
        ...
        public LPModel ToStandardForm()
        {
            ...
            // Map: original var index -> list of (newVarIndex, signMultiplier)
            // For Positive: one mapping (newIdx, +1)
            // For Negative: one mapping (newIdx, -1) with y = -x >= 0
            // For Unrestricted: two mappings (x+, +1), (x-, -1)
            var variableMapping = new Dictionary<int, List<(int index, double sign)>>();

            for (int i = 0; i < Variables.Count; i++)
            {
                var originalVar = Variables[i];
                variableMapping[i] = new List<(int, double)>();
                ...
                switch (originalVar.Type)
                {
                    case VariableType.Unrestricted:
                    {
                        ...
                        variableMapping[i].Add((xPlus.Index, +1.0));
                        variableMapping[i].Add((xMinus.Index, -1.0));
                        break;
                    }
                    case VariableType.Negative:
                    {
                        // x <= 0 -> substitute x = -y, y >= 0
                        var y = standardModel.AddVariable($"{originalVar.Name}_neg", -originalVar.Coefficient, VariableType.Positive);
                        variableMapping[i].Add((y.Index, -1.0));
                        break;
                    }
                    default:
                    {
                        // Positive / Integer / Binary are already >= 0 by default bounds
                        var newVar = standardModel.AddVariable(originalVar.Name, originalVar.Coefficient, originalVar.Type);
                        variableMapping[i].Add((newVar.Index, +1.0));
                        break;
                    }
                }
            }

            // Transform constraints to standard form (<= with nonnegative RHS) and expand coefficients for split/substituted variables
            foreach (var originalConstraint in Constraints)
            {
                var (coeffs, relation, rhs, _) = originalConstraint.ToStandardForm();

                // Build expanded coefficient row for standard model
                var expandedCoeffs = new List<double>();

                for (int i = 0; i < Variables.Count; i++)
                {
                    var mappings = variableMapping[i];

                    if (mappings.Count == 1)
                    {
                        // Single variable mapping (positive/continuous/integer/binary or negative with sign)
                        var (newIdx, sign) = mappings[0];
                        // We append in order of creation; expandedCoeffs must align with standardModel.Variables order.
                        // We'll collect into a temp array across all vars below, to ensure alignment.
                    }
                }

                // Because standardModel variables were created in the loop above (in order), we can fill a full row aligned to its count:
                var fullRow = new double[standardModel.VariableCount];
                for (int i = 0; i < Variables.Count; i++)
                {
                    var mappings = variableMapping[i];
                    foreach (var (newIdx, sign) in mappings)
                    {
                        fullRow[newIdx] += sign * coeffs[i];
                    }
                }

                standardModel.AddConstraint($"{originalConstraint.Name}_std", fullRow.ToList(), relation, rhs);
            }
        ...
        /// <summary>
        /// Converts maximization to minimization or visa versa
        /// </summary>
        ...
        public string ToFormulationString()
        {
            ...
            for (int i = 0; i < Variables.Count; i++)
            {
                var coef = Variables[i].Coefficient;
                if (i > 0)
                {
                    sb.Append(coef >= 0 ? " + " : " ");
                }
                sb.Append($"{coef:F3} {Variables[i].Name}");
            }
            sb.AppendLine();
