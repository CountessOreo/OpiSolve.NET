...
namespace OptiSolver.NET.Core
{
    ...
    public class Constraint
    {
        ...
        public double GetViolation(double[] variableValues)
        {
            double lhsValue = EvaluateLHS(variableValues);

            return Relation switch
            {
                ConstraintRelation.LessThanOrEqual => Math.Max(0, lhsValue - RightHandSide),
                ConstraintRelation.Equal => Math.Abs(lhsValue - RightHandSide),
                ConstraintRelation.GreaterThanOrEqual => Math.Max(0, RightHandSide - lhsValue),
                _ => throw new ArgumentException($"Unknown constraint relation: {Relation}")
            };
        }
        ...
        public override string ToString()
        {
            ...
            sb.Append($" {EnumHelper.RelationToString(Relation)} {RightHandSide:F1}");
